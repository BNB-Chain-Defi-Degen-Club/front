...
export const ImmigratePanel = () => {
  ...
  const handleSaveButton = () => {
    const textarea = document.getElementById('immigration-code') as HTMLInputElement;
    if (
      textarea &&
      window.confirm('Are you sure you want to override the statistics on this device? This action is not reversable.')
    ) {
      const migrationStats = JSON.parse(decrypt(textarea.value) ?? '') as MigrationStats;
  ...
  return (
    <div className="text-sm text-gray-500 dark:text-gray-300">
      <label htmlFor="message" className="mb-2 block text-left text-sm font-medium text-gray-900 dark:text-gray-400">
      ...
      >
        {isSaveButtonEnabled && <SaveIcon className="mr-2 h-6 w-6 cursor-pointer dark:stroke-white" />}
        Save
      </button>
    </div>
  );
